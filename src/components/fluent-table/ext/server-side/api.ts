...
export interface RestApiResponse<TData = unknown> {
  data: TData[];
 ...
 */
export function transformRestApiResponse<TData = unknown>(
  response: unknown
): ServerResponse<TData> {
  // Type guard to check if response matches expected structure
  function isValidRestResponse(obj: unknown): obj is RestApiResponse<TData> {
 ...
 */
export function isRestApiFormat(response: unknown): boolean {
  try {
 ...
 */
export function createRestApiResponse<TData = unknown>(
  data: TData[],
  recordsFiltered: number,
  recordsTotal?: number
): RestApiResponse<TData> {
  if (!Array.isArray(data)) {
 ...
 */
export function formatRestApiRequest(params: ServerSideParams): {
  body: string;
...
export async function makeRestApiRequest(
  ...
  const { body, headers } = formatRestApiRequest(params);
 ...
 */
export function createRestApiQueryFn<TData = unknown>(url: string) {
  return async (params: ServerSideParams): Promise<ServerResponse<TData>> => {
    const response = await makeRestApiRequest(url, params);
